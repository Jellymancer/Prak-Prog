...
void fittry(gsl_vector* c, double* x, double* y, double* dy, double m, gsl_vector* dc){//Printing the fit using the found parameters.
                double dx=0.1;
...
FILE* ogdata; // printing original data
...
gsl_vector* dc=gsl_vector_calloc(m);
gsl_matrix* Sigma=gsl_matrix_calloc(m,m);

matrix_generation(M,b,x,y,dy);
lsfit(M,b,c,Sigma);

for(int i=0;i<m;i++) gsl_vector_set(dc,i,sqrt(gsl_matrix_get(Sigma,i,i)));
...
printm(Sigma,opg2);
fprintf(opg2,"The errors on the fitting parameters are the squares of the diagonal elements of the covariance matrix.\n [dc0;dc1;dc2]=\n");
printv(dc,opg2);
fprintf(opg2,"Best fit is plotted together with the maximum/minimum fits (using maximum/minimum values of the fit parameters).\n");
return 0;}
